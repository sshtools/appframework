...
public class SwingVirtualSessionManager extends JPanel implements
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						if (tabs != null) {
							...
							if (idx != -1 && idx < tabs.getComponentCount()) {
	...
	public VirtualSession getSelectedVirtualSession() {
		if (tabs != null) {
			...
			// return sel == -1 ? null : ( sel >= virtualSessions.size() ?
			// (VirtualSession)virtualSessions.elementAt(virtualSessions.size()
			// - 1) : (VirtualSession)virtualSessions.elementAt(sel) );
			return sel == -1 || sel >= virtualSessions.size() ? null
				: (VirtualSession) virtualSessions.get(sel);
		}
