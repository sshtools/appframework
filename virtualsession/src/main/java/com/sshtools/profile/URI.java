...
public class URI implements Serializable {
	 ...
	 * 
	 * @param p_uriSpec the URI specification string (cannot be null or empty)
	 * @param allowNonAbsoluteURI true to permit non-absolute URIs, false
	 *            otherwise.
	 *
	 * @exception MalformedURIException if p_uriSpec violates any syntax rules
	 */
	 ...
	 *            however, if the query and fragment are specified both in the
	 *            path and as separate parameters, an exception is thrown
	 ...
	 *
	 * @exception MalformedURIException if p_base is null and p_uriSpec is not
	 *                an absolute URI or if p_uriSpec violates syntax rules
	 */
		...
		if (host.length() > 0) {
			...
			if (hasPort) {
				...
				if (portStr.length() > 0) {
					 ...
					 * MalformedURIException( portStr + " is invalid. Port
	 ...
	 *
	 * @exception MalformedURIException if p_scheme is not a conformant scheme
	 *                name
	 */
	 ...
	 *
	 * @exception MalformedURIException if p_host is not a valid IP address or
	 *                DNS hostname.
	 */
	 ...
	 * 
	 * @exception MalformedURIException it authority is not a well formed
	 *                registry based authority
	 */
	 ...
	 *                URI does not conform to the generic URI syntax or if the
	 *                path is null
	 */
	 ...
	 *
	 * @exception MalformedURIException if p_fragment is not null and this URI
	 *                does not conform to the generic URI syntax or if the path
	 *                is null
	 */
	...
	public boolean equals(Object p_test) {
		if (p_test instanceof URI) {
			...
			if (((m_scheme == null && testURI.m_scheme == null)
					...
					&& ((m_regAuthority == null && testURI.m_regAuthority == null) || (m_regAuthority != null
							&& testURI.m_regAuthority != null && m_regAuthority.equals(testURI.m_regAuthority)))
	...
	private static int scanHexSequence(String address, int index, int end, int[] counter) {
		...
		for (; index < end; ++index) {
			...
			// This might be an interface index
			else if (testChar == '%') {
				int cidx = index + 1;
				while (cidx < end && isDigit(address.charAt(cidx))) {
					cidx++;
				}
				if (cidx > end || cidx == (index + 1) || address.charAt(cidx) != ']') {
					return -1;
				}
				index = cidx;
			}
	...
	// return (p_char <= ']' && (fgLookupTable[p_char] & RESERVED_CHARACTERS) !=
	// 0);
	// }
	...
	// return (p_char <= '~' && (fgLookupTable[p_char] & MASK_UNRESERVED_MASK)
	// != 0);
	// }
	/**
