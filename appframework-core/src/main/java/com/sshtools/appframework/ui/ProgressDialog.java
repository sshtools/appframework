...
public class ProgressDialog extends JDialog {
	...
	public static ProgressDialog createDialog(Component parent, Icon icon, String geometryPropertyName,
			AppAction action, String label1Text, String title, boolean modal, boolean closeEnabled) {
		Window w = (Window) SwingUtilities.getAncestorOfClass(Window.class, parent);
		ProgressDialog progressDialog = null;
		if ((w != null) && w instanceof JDialog) {
			progressDialog = new ProgressDialog((JDialog) w, icon, modal, action, label1Text, title,
					geometryPropertyName, closeEnabled);
		} else if ((w != null) && w instanceof JFrame) {
			progressDialog = new ProgressDialog((JFrame) w, icon, modal, action, label1Text, title,
					geometryPropertyName, closeEnabled);
		} else {
			progressDialog = new ProgressDialog((JFrame) null, icon, modal, action, label1Text, title,
					geometryPropertyName, closeEnabled);
		}
	...
	public static ProgressDialog createDialog(Component parent, Icon icon, String geometryPropertyName,
			String label1Text, String title, boolean closeEnabled) {
	...
	public void setIndeterminate(boolean indeterminate) {
		try {
			progressBar.getClass().getMethod("setIndeterminate", new Class[] { boolean.class }).invoke(progressBar,
					new Object[] { Boolean.valueOf(indeterminate) });
		} catch (Exception e) {
	...
	private void init(Icon icon, AppAction action, String label1Text, String geometryPropertyName,
